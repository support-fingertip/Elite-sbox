trigger AccountTrigger on Account (before insert,after insert,before update,after update,after delete) {
    if(Label.ActivateAccountTrigger =='True')
    {
        if(Trigger.isInsert)
        {
            if(Trigger.isBefore)
        ...
        if(trigger.isUpdate)
        {
            ...
            else
            {
                ...
                for(Account acc: trigger.new){
                    if(acc.Customer_Status__c == 'Inactive' && acc.Customer_Status__c != trigger.oldMap.get(acc.Id).Customer_Status__c && acc.Deactivate_Reason2__c != 'Replacement'){
                        accIds.add(acc.Id);
                    }
                }
                if(accIds.size() > 0){
