...
handleExportXlsx() {
    ...
    try {
        ...
        // Generate real XLSX binary and download via Blob
...
// ─────────────────────────────────────────────
// Export PDF — opens printable HTML via Blob URL
// blob: URLs are same-origin, not blocked like data: URIs
// ─────────────────────────────────────────────
handleExportPdf() {
    this.showDownloadMenu = false;
    if (!this.allLedgerData || this.allLedgerData.length === 0) {
    ...
    try {
        ...
        if (printWindow) {
            ...
            // Fallback: download as HTML file if popup still blocked
            URL.revokeObjectURL(blobUrl);
            const downloadBlob = new Blob([htmlContent], { type: 'application/octet-stream' });
            ...
            this.showToast('Info', 'PDF print was blocked. HTML file downloaded — open it and press Ctrl+P to save as PDF.', 'info');
        }
