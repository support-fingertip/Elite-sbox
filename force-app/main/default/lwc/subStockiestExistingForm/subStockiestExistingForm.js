...
import saveSubStockistExistingMarketVisit from '@salesforce/apex/VisitFormController.saveSubStockistExistingMarketVisit';
...
export default class ExistingSecondaryCustomer extends NavigationMixin(LightningElement) {
    ...
    customerTypeOptions = [{ label: 'New Sub Stockiest', value: 'New Sub Stockiest' }];
    ...
    accountFilter = { 
        criteria: [
            ...
            {
                fieldPath: "Primary_Customer_Type__c",
                operator: "eq",
                value: "Superstockiest"
            }
        ]
    };
    ...
    handleGetLatLon() {
        ...
        if (!this.subStockiestName) {
            this.showToast('Error', 'Please enter Sub Stockiest Name', 'error');
            return;
        }
        ...
        if (!this.customerImageUploaded) {
            this.showToast('Error', 'Please upload Sub Stockiest image', 'error');
            return;
        }
