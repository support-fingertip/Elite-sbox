...
export default class Testing extends NavigationMixin(LightningElement) {
    ...
    @track showusers = false; // Show Users table
    @track userList = [];
    ...
    resetData() {
        ...
        this.showSecoundaryOrders = false;
    ...
    fetchUsers() {
        ...
        getUsers({
            ...
            .then(result => {
                ...
                this.allUsers = mappedUsers;          // ✅ backup
                this.userList = [...mappedUsers];     // ✅ UI list
    ...
    OrderData() {
        ...
        ORD_DATA({
            ...
            .then(result => {
                ...
                this.ordFilter.statusVal = result.statusOptions;
                this.ordFilter.allordData = uniqueOrders;
                this.ordFilter.isOrderDataExisted = uniqueOrders.length !== 0;
                this.ordFilter.originalOrdData = uniqueOrders;
                this.isSubPartLoad = false;
    ...
    handleOrdSerch(event) {
        ...
        if (txt.length > 0) {
            const ordUpdate = this.ordFilter.originalOrdData.filter(ord =>
            ...
            // Ensure unique orders in search results
            const uniqueOrders = [];
            const orderIds = new Set();
            ordUpdate.forEach(order => {
                if (!orderIds.has(order.orderId)) {
                    orderIds.add(order.orderId);
                    uniqueOrders.push(order);
                }
            ...
            this.ordFilter = {
                ...this.ordFilter,
                allordData: this.ordFilter.originalOrdData,
                isOrderDataExisted: this.ordFilter.originalOrdData.length !== 0
            };
        }
    }
    ...
    handleInvSerch(event) {
        ...
        if (txt.length > 0) {
            ...
            this.InvFilter = {
                ...this.InvFilter,
                allInvData: dataUpdate,
                isInvoiceDataExisted: dataUpdate.length !== 0
            };
        } else {
            this.InvFilter = {
                ...this.InvFilter,
                allInvData: this.InvFilter.originalInvData,
