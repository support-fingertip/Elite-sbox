...
import saveNewPrimaryCustomerVisit from '@salesforce/apex/VisitFormController.saveNewPrimaryCustomerVisit';
...
import Primary_Customer_Business_FIELD from '@salesforce/schema/Visit_Form__c.Primary_Customer_Business_Type__c';
...
export default class ExistingSecondaryCustomer extends NavigationMixin(LightningElement) {
    ...
    @track customerType = 'New Primary Customer';
    @track visitType = '';
    @track primaryCustomerBusinessType = '';
    ...
    @wire(getPicklistValues, {
        ...
        fieldApiName: Primary_Customer_Business_FIELD
    })
    ...
    connectedCallback() {
        this.customerTypeOptions = [{ label: this.customerType, value: this.customerType }];
        getAllowedProducts()
        .then(result => {
    ...
    handleGetLatLon() {
        ...
        if (!this.primaryCustomerName) {
            this.showToast('Error', 'Please enter Primary Customer Name', 'error');
            return;
        }
        ...
        if (!this.primaryCustomerBusinessType) {
            this.showToast('Error', 'Please select primary customer bussiness type', 'error');
            return;
        }
    ...
    async handleSubmit() {
        ...
        saveNewPrimaryCustomerVisit({
            primaryCustomerName: this.primaryCustomerName,
            businessType: this.primaryCustomerBusinessType,
